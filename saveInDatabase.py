def saveInDatabase(json_file):
    ...
    (Title TEXT, Year INTEGER, runTime INTEGER, metaScore REAL, imdbRating REAL, genre TEXT)''')

    cur.execute('SELECT Title FROM MovieInfo WHERE Title = ?', (title,))
    row = cur.fetchone()

    if row is None:
        cur.execute('''INSERT INTO MovieInfo (Title, Year, runTime, metaScore, imdbRating, genre)
                VALUES (?,?,?,?,?,?)''', (title, year, runTime, metaScore, imdbRating, genre))
    else:
        print("Database already contains the query.")
